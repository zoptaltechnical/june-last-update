...
-(instancetype)initWithFrame:(CGRect)frame
         ...
         singleSelectionOnly:(BOOL)singleSelection disableDates:(NSMutableArray*)disableDates
{
    ...
    if(self = [self initWithFrame:frame collectionViewLayout:flow])
    {
        ...
        self.disableDates=disableDates;
...
-(UICollectionViewCell*)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.row + 1 >= weekDay && indexPath.row + 1 <= weekDay-1 + numOfDays)
    {
        ...
        NSString *stringFromDate = [formatter stringFromDate:cell.attachedDate];
        
        
        if ([_disableDates containsObject:stringFromDate])
        {
            [cell setDateDisable:YES];
        }
        else
        {
            [cell setDateDisable:NO];
...
-(void)selectDisableDates:(NSSet*)dates
{
    NSMutableArray* datesToUpdate = [NSMutableArray new];
    ...
    {
        NSDate* dateToSelect = [self clearDateFromhhmmss:date];
        [self.disableDates addObject:dateToSelect];
        [datesToUpdate addObject:dateToSelect];
    }
